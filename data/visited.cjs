const { con } = require('../lib/db'); // import for db connection
...
function initialiseVisits(userID) {
  Object.keys(abr2name).forEach(element => {
    con.query(`SELECT * FROM visited_status WHERE userID = ${userID} and countryAbr = '${element}'`, (error, rows) => {
      if (rows.length == 0) {
        const row = { userID: userID, countryAbr: element, countryName: abr2name[element], visited: 0 };
        con.query('INSERT INTO visited_status SET ?', row, (err, res) => {
          if (err) throw err;
...
function getVisited(userID) {
  return new Promise(function (resolve, reject) {
    con.query(`SELECT countryAbr, visited FROM visited_status WHERE userID = ${userID}`, (error, rows) => {    
      rows.forEach(row => {
